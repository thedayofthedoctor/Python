...
def fun_get_titl(open_chwp,inpt_link):#获得作品标题，总页码，并根据网站区分标题行写法.
    ...
    if sep_link == 'iimanhua':
        ...
        if jdg_link =='':
            ...
            chp_name=who_titl.replace(tit_list[0]+' ','').replace(tit_list[-1],'').replace(tit_list[-2],'').replace(' ','-').replace('--','')
...
def fun_get_urls(inpt_link):#整理全部章节连接，显示章节数目以及爬虫记录.
    ...
    str_urls='共发现:'+str(num_urls)+'个链接.'
    str_link='经筛选有:'+str(num_link)+'个链接符合要求.'
    print(str_coti+'\n'+str_urls+'\n'+str_link)
...
def fun_get_chap(dict_link):
    ...
    for link in lst_link:
        ...
        get_driv.close()
        sed_time=time.time()
        stt_time=format(sed_time-sst_time,'.3f')
